...
class Minimizer:
    ...
    def __init__(self, userfcn, params, fcn_args=None,batch = None,run_type = None , fcn_kws=None,
    ...
    def __residual(self, fvars, apply_bounds_transformation=True):
        ...
        if self.run_type == 'batch':
            out = np.array([])
            class_fetch = self.fetch
            while True:
                fetch = class_fetch('df_')
                temp = fetch.data
            
                # Calculating  residue 
                array = self.userfcn(params, *self.userargs ,temp)
                del temp
                out = np.concatenate((out,array), axis = 0)
                #out.append(array)
            
                path = 'df_'
                if not os.path.exists(path + str(class_fetch.count) +  '.csv'):
                    class_fetch.count = 1
                    break    
                    
        if self.run_type == None:
